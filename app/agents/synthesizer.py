...
async def call_synthesizer(
    ...
    for i, fact in enumerate(all_findings, 1):
        context += f"[{i}] {fact.get('fact', '')} (source: {fact.get('source', '')})\n"

    logger.info(f"Synthesizer calling LLM: provider={provider} model={model} with {len(all_findings)} facts")
    messages = [
